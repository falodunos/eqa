...
function LoadCalendarScript(callback){
	function LoadFullCalendarScript(){
		if(!$.fn.fullCalendar){
			$.getScript('admin/plugins/fullcalendar/fullcalendar.js', callback);
		}
		else {
	...
	if (!$.fn.moment){
		$.getScript('admin/plugins/moment/moment.min.js', LoadFullCalendarScript);
	}
	else {
...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript('admin/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript('admin/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSelect2Script(callback){
	if (!$.fn.select2){
		$.getScript('admin/plugins/select2/select2.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript('admin/plugins/datatables/jquery.dataTables.js', function(){
			$.getScript('admin/plugins/datatables/ZeroClipboard.js', function(){
				$.getScript('admin/plugins/datatables/TableTools.js', function(){
					$.getScript('admin/plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('admin/plugins/fineuploader/jquery.fineuploader-4.3.1.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript('admin/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('admin/plugins/d3/d3.v3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript('admin/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript('admin/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript('admin/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript('admin/plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript('admin/plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript('admin/plugins/fancybox/jquery.fancybox.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript('admin/plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript('admin/plugins/sparkline/jquery.sparkline.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function TestTable3(){
	$('#datatable-3').dataTable( {
		...
		"oTableTools": {
			"sSwfPath": "admin/plugins/datatables/copy_csv_xls_pdf.swf",
			"aButtons": [
				"copy",
				"print",
				{
...
//	
//	//THE DASHBOARD IS BEIND LOADED FROM ZEND adminController indexAction
// 	if (ajax_url.length < 1) {
//		ajax_url = 'admin/ajax/dashboard.phtml';
//	}
//	LoadAjaxContent(ajax_url);

	$('.main-menu').on('click', 'a', function (e) {
		var currentTag = $(this);
		if(currentTag.parent().hasClass('dropdown')){	
			e.preventDefault();
	...
	$('#search').on('keydown', function(e){
		if (e.keyCode == 13){
			e.preventDefault();
			$('#content').removeClass('full-content');
			ajax_url = 'admin/ajax/page_search.phtml';
			window.location.hash = ajax_url;
			LoadAjaxContent(ajax_url);
		}
	});
